  ...
  try {
    ...
    if (question?.type === 'calculation') {
      ...
      if (blanks.length === 0) return (
        <p className="text-xs text-gray-500">（このクイズの正解データが未設定です）</p>
      )
      return (
...
function StudentAnswerDisplay({ question, studentAnswer }: { question: Question; studentAnswer: unknown }) {
  try {
    const ans = studentAnswer as StudentAnswer
    ...
    if (question.type === 'calculation' && ans.type === 'calculation') {
      const blanks = question.blanks ?? []
      if (blanks.length === 0) return <p className="text-xs text-gray-500">（未回答）</p>
      return (
        <p className="text-xs text-gray-700 leading-relaxed">
          {blanks.map(b => `${b.position}：${ans.blanks?.[b.position] ?? '（空白）'}`).join('　')}
        </p>
      )
    }

    if (question.type === 'journal_entry' && ans.type === 'journal_entry') {
      const rows = ans.rows ?? []
      if (rows.length === 0) return <p className="text-xs text-gray-500">（未回答）</p>
      return (
        <div className="overflow-x-auto">
          <table className="border-collapse text-xs w-full">
            <thead>
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={i}>
                  <td className="border border-gray-300 px-2 py-1 text-center">{row?.debit_account}</td>
                  <td className="border border-gray-300 px-2 py-1 text-right">{row?.debit_amount}</td>
                  <td className="border border-gray-300 px-2 py-1 text-center">{row?.credit_account}</td>
                  <td className="border border-gray-300 px-2 py-1 text-right">{row?.credit_amount}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    if (question.type === 'description' && ans.type === 'description') {
      return <p className="text-xs text-gray-700">{ans.text ?? '（未回答）'}</p>
    }
  } catch (e) {
    console.error('StudentAnswerDisplay error:', e)
    return <p className="text-xs text-gray-400">（表示エラー）</p>
  }
  return null
}
...
export default function QuizPage() {
  ...
  const handleSubmit = async (submittedAnswers: Record<string, StudentAnswer>) => {
    ...
    try {
      ...
      if (data.save_debug) setSaveDebug(data.save_debug)
      setPhase('result')
    } catch (e) {
                ...
                <div key={index} className={`bg-white rounded-xl border-l-4 p-4 ${detail.is_correct ? 'border-green-500' : 'border-red-500'}`}>
                  ...
                  {q && (
                    <div className="bg-gray-50 rounded-lg p-3 mb-2">
                      <p className="text-gray-600 font-medium text-xs mb-1">あなたの回答:</p>
                      <StudentAnswerDisplay question={q} studentAnswer={detail.student_answer} />
                    </div>
                  )}
                  {!detail.is_correct && q && (
